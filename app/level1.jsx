...
import level1Questions from '../assets/questions/level1';
  ...
  useEffect(() => {
    const fetchHighScore = async () => {
      try {
        ...
        if (userSnap.exists()) {
          const level1HighScore = userSnap.data().highscores.level1;
          setHighScore(level1HighScore);
        }
  ...
  const handleAnswer = async (isCorrect) => { 
    ...
    if (currentIndex < level1Questions.length - 1) {
      ...
      router.replace({ 
        ...
        params: { finalScore, highScore }, // send scores to results screen to display
      });
    } 
    if (isCorrect) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.card}>
          <Text style={styles.text}>
            {level1Questions[currentIndex].question}
          </Text>
        </View>
        
        <View style={styles.questionContainer}>
          <View style={styles.questionContainer}>
            {level1Questions[currentIndex].answers.map((answer, index) => ( 
            <TouchableOpacity key={index} style={styles.answerButton} onPress={() => 
